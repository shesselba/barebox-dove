...
static void port_init_rxdesc_ring(struct port_priv *port)
{
	...
	for (n = 0; n < RX_RING_SIZE; n++) {
		nxtdesc = rxdesc + ALIGN(sizeof(*rxdesc), 32);

		rxdesc->cmd_sts = RXDESC_OWNED_BY_DMA;
		rxdesc->buf_size = ALIGN(PKTSIZE, 32);
		rxdesc->byte_cnt = 0;
...
static int port_probe(struct device_d *parent, struct port_priv *port)
{
	...
	port->txdesc = dma_alloc_coherent(ALIGN(sizeof(*port->txdesc), 32));
	port->rxdesc = dma_alloc_coherent(RX_RING_SIZE *
					  ALIGN(sizeof(*port->rxdesc), 32));
	port->rxbuf = dma_alloc(RX_RING_SIZE * ALIGN(PKTSIZE, 32));
	...
	writel((u32)port->rxdesc, port->regs + PORT_CRDP(URXQ));
